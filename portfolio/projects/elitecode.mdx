...
1. **Voice interfaces need good feedback** - Users need to know they're being heard and understood. Visual feedback during recording is essential. Showing real-time transcription helps users feel confident their input is being captured correctly.

2. **Transcription quality varies** - Technical vocabulary and speaking style affect accuracy. Providing correction mechanisms improves user experience. Whisper handles coding terminology surprisingly well, but edge cases still occur.

3. **Natural voice matters** - Robotic text-to-speech breaks immersion. Investing in natural voice synthesis with ElevenLabs significantly improves the experience and makes the feedback feel more like a conversation with a real person.

4. **Interview prep is emotional** - Users are often anxious. Framing feedback constructively and encouragingly is as important as accuracy. The goal is to build confidence, not to criticize—coding interviews are hard enough without fumbling through explanations.

5. **Hackathons force rapid learning** - Building EliteCode at SFHacks 2023 required quickly integrating multiple APIs (Whisper, OpenAI, ElevenLabs) into a coherent product. The time constraint forced us to focus on what mattered most: a working demo that demonstrated the core concept.
